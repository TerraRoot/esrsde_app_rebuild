...
printf "\n"
devices=$(curl --silent -X POST "http://localhost:5984/device/_find" -H "Authorization: Basic =" -H "accept: application/json" -H "Content-Type: application/json" -d  "{\"selector\": { \"_id\" : {\"\$regex\" : \"device-\"}}, \"fields\": [  \"_id\",\"_rev\" ]}" | grep -E "^{\"_id")
...
for d in ${devices[@]}; do
   ...
   curl -X DELETE "http://localhost:5984/device/$id?rev=$rev" -H "Authorization: Basic ="
done
